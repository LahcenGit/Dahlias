                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="basic-form">
                            ...
                            <form action="{{url('/dashboard-admin/final-registrations/'.$registration->id)}}" method="POST" enctype="multipart/form-data">
                            <input type="hidden" name="_method" value="PUT">
                                @csrf
                                <div class="form-row">
                                    <div class="form-group col-md-6">
                                        <label>Etudiants* :</label>
                                        <select type="text"  class="form-control invalid"  class="selectpicker"  data-live-search="true" name="student"  required>
                                          @foreach($students as $student)
                                           <option value="{{$student->id}}" @if($registration->user_id == $student->id )selected @endif>{{$student->name}}</option>
                                          @endforeach
                                        </select>
                                    </div>
                                 </div>
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <select type="text"  class="form-control invalid"  class="selectpicker"  data-live-search="true" name="course" id="select-course" required>
                                          @foreach($courses as $course)
                                           <option value="{{$course->id}}" @if($registration->course_id == $course->id )selected @endif>{{$course->name}}</option>
                                          @endforeach
                                        </select>
                                    </div>
                                 </div>
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>Edition* :</label>
                                         ...
                                         @foreach($editions as $edition)
                                          <option value="{{$edition->id}}" @if($registration->group_id == $edition->id )selected @endif>{{$edition->group}}</option>
                                         @endforeach
                                        </select>
                                    </div>
                                </div>
